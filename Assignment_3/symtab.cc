...
symbol_t * symbol_create (symtab_t * symtab, string p_name, int p_type)
{
  ...
  switch (p_type)
  {
    case DTYPE_INT: symtab->offset += sizeof(int);
      break;
    case DTYPE_FLOAT: symtab->offset += sizeof(float);
      break;
...
{
  cout << sym->name << " |\t " << sym->addr << " |\t " << get_type_str (sym) << endl;
}
